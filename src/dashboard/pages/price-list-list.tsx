            ...
            customizeColumns={
                {
                    ...
                    channels: {
                        header: t`Channels`,
                        // Join the channel codes with a separator. Without a
                        // custom cell ListPage renders the relation array as
                        // raw JSON; Vendure channels have no `name`, so `code`
                        // is the human-readable identifier.
                        cell: ({ row }: any) => {
                            const codes = (row.original.channels ?? []).map(
                                (c: any) => c.code,
                            );
                            return codes.length ? (
